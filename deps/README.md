...
2. Open the `.dmg`  
3. Drag `Multicultivator-macos-arm64.app` or `Multicultivator-macos-x64.app` into Applications  
...
2. Run the installer  
3. Start the application from the desktop or start menu  
...
The input file must be a tab separated `.txt` file.
...
Cylinder mapping is inferred from naming patterns such as `-1-`, `-2-`, ..., `-8-`.

Each cylinder must contain:
...
Results are automatically written next to the selected input file.
...
- CSV tables of growth phases  
- HTML files with interactive plots  
- Debug log file (if errors occur)  
